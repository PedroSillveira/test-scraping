...
# Define um cabeçalho User-Agent para simular um navegador e evitar bloqueios do site
headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
}

# Faz uma requisição HTTP para a página de pesquisa do Mercado Livre com o produto desejado
response = requests.get(url_base + produto_pesquisa, headers=headers)
# Converte o conteúdo da resposta para um objeto BeautifulSoup para facilitar a extração dos dados
site = BeautifulSoup(response.text, 'html.parser')
# Encontra todos os produtos listados na página
produtos = site.findAll('div', attrs={'class': 'poly-card poly-card--list'})

# Percorre cada produto encontrado na página
for produto in produtos:
    ...
    print(price.text)
    # Adiciona um espaçamento entre os produtos para melhor visualização
    print('\n\n')